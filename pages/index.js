...
export default function Home() {
  ...
  const STAGE_OPTIONS = [/* … same as before … */];
  const ASSESSMENT_OPTIONS = [/* … same as before … */];

  // Group traits
  const groupedTraits = { Qualitative: [], Quantitative: [], "Pseudo-qualitative": [] };
  Object.entries(traitData).forEach(([k, t]) => groupedTraits[t.type]?.push(k));
